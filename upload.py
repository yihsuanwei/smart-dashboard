...
# ===== MCID 彈窗 =====
@st.dialog("MCID 設定")
def mcid_dialog(seller_keys):
    """上傳後或手動觸發的 MCID 編輯彈窗"""
    registry = load_seller_registry()
    st.caption("輸入各賣家的 MCID，按 Enter 後點儲存")

    updated = {}
    for sk in seller_keys:
        display = sk.replace('_', ' ')
        val = st.text_input(
            display,
            value=registry.get(sk, ""),
            key=f"mcid_dlg_{sk}",
        )
        updated[sk] = val.strip()

    if st.button("儲存", type="primary", use_container_width=True):
        for sk, mcid in updated.items():
            registry[sk] = mcid
        save_seller_registry(registry)
        st.session_state['mcid_saved_toast'] = True
        st.rerun()
...
with col1:
    ...
    if uploaded_files:
        ...
        if st.button("全部上傳並處理", type="primary", use_container_width=True, icon=":material/cloud_upload:"):
            ...
            # 從這次上傳的檔名提取賣家
            uploaded_sellers = set()
            for cfg in file_configs:
                seller = _extract_seller_name(cfg["custom_filename"] + ".csv", cfg["file_type"])
                if seller:
                    uploaded_sellers.add(seller)
            if uploaded_sellers:
                st.session_state['mcid_dialog_sellers'] = sorted(uploaded_sellers)

            st.session_state['uploader_key'] += 1  # 重置 file_uploader
            st.rerun()
    ...
    if 'upload_results' in st.session_state:
        ...
        # 用 toast 顯示上傳結果（幾秒後自動消失）
        if fail_count == 0:
            st.toast(f"全部完成，共 {success_count} 個檔案")
        else:
            st.toast(f"完成 {success_count} 個，失敗 {fail_count} 個")
            for success, msg in results:
                if not success:
                    st.toast(msg)

    # === MCID 儲存成功 toast ===
    if 'mcid_saved_toast' in st.session_state:
        st.session_state.pop('mcid_saved_toast')
        st.toast("MCID 已儲存")

    # === 上傳後自動跳出 MCID 彈窗 ===
    if 'mcid_dialog_sellers' in st.session_state:
        sellers_to_tag = st.session_state.pop('mcid_dialog_sellers')
        mcid_dialog(sellers_to_tag)

    # === 補填 MCID 按鈕（針對已上傳但沒填的賣家）===
    all_sellers = scan_sellers_from_files()
    if all_sellers:
        if st.button("MCID 管理", key="mcid_manage_btn"):
            mcid_dialog(all_sellers)
